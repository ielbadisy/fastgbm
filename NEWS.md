...
* Changed feature subsampling (`colsample`) from once-per-tree (xgboost's
  ...
  semantics, resampled at every split): `src/survgbm.cpp`'s `build_tree()`
...
* Changed `survgbm()`'s default hyperparameters to match what all this
  ...
  explicitly in `?survgbm` that training all `ntrees` rounds **without**
...
* `inst/benchmarks/run-benchmark.R` now benchmarks all three implemented
  objectives (`survgbm_cox`, `survgbm_aft`, `survgbm_pexp`) as separate arms
...
* Explored (not formalized into the package): an `mtry`-scaled-to-`sqrt(p)/p`
  ...
  bagged-ensemble-of-`survgbm`-fits wrapper (bootstrap resampling + averaging,
  ...
  others. See Roadmap in `paper/survgbm-benchmark.qmd`.
...
* Added a piecewise-exponential (PEM) objective. Unlike Cox/AFT (one scalar
  ...
  the standard "Poisson trick" (`survgbm_expand_person_time()`, `R/pexp.R`),
  ...
  (`compute_pexp_grad_hess()`/`compute_pexp_loss()`, `src/survgbm.cpp`) -- no
...
* **Bug fix** (found while building `pexp`, applies to all objectives): the
  ...
  matrix's actual `colnames()`. Fixed in `src/survgbm.cpp`.
* RMST-oriented loss and a Royston-Parmar-style objective remain undesigned
  (see Roadmap in `paper/survgbm-benchmark.qmd`); `pexp` was implemented
...
* **Implemented validation-based early stopping** (`validation`/`early_stopping`
  ...
  `compute_cox_loss()`/`compute_aft_loss()` to `src/survgbm.cpp`, extended
  `survgbm_fit_cpp()` to bin a validation set with the training cuts, track validation
...
* Rebenchmarked all 6 datasets with early stopping + the new `colsample` default:
  ...
  time dropped 4-10x since far fewer trees are actually grown -- `survgbm` is now
...
* Repeated the regularization probe with early stopping active (it had only been run
  ...
  Rebenchmarked again: `survgbm` now beats both `gbm` and `xgboost` on C-index on 4 of
...
* Package renamed `fastgbm` → `survgbm`. All identifiers, exported functions, and the
  compiled backend (`src/survgbm.cpp`) were renamed to match.
* Removed `reg:squarederror` and `binary:logistic` objectives entirely: `survgbm` is
...
* Added `RcppParallel` as a compiled dependency and parallelized the per-feature
  split search (`SplitFinder` worker in `src/survgbm.cpp`): each feature's best split
...
* **Bug fix**: `survgbm_survival_baseline()` (the Breslow baseline cumulative
...
* **Bug fix**: `importance()` crashed (or silently returned garbage
  ...
  `feature_names`.
* **Bug fix**: `survgbm()` did not validate that `nrow(x)` matched
...
* Added `tests/testthat/test-gradients.R` (finite-difference checks for all
  four objectives, via a new internal `survgbm_grad_hess()` test hook onto
