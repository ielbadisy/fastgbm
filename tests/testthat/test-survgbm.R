test_that("serialization round trips", {
  ...
  time <- c(1, 2, 3, 4)
  status <- c(1, 1, 0, 1)
  fit <- survgbm(x, time = time, status = status, objective = "cox", ntrees = 2L,
...
test_that("formula interface expands factors", {
  ...
  fit <- survgbm(survival::Surv(time, status) ~ x1 + x2, data = dat, objective = "cox",
...
test_that("cox survival objective trains and predicts survival curves", {
  ...
  fit <- survgbm(x, time = dat$time, status = dat$status, objective = "cox", ntrees = 5L,
...
test_that("aft survival objective trains and predicts survival curves", {
  ...
  fit <- survgbm(x, time = time, status = status, objective = "aft", ntrees = 5L,
...
test_that("survgbm() requires data when x is a formula", {
  skip_if_not_installed("survival")
  expect_error(survgbm(survival::Surv(time, status) ~ x1), "`data`")
})

test_that("survgbm() rejects a non-Surv formula response", {
  dat <- data.frame(y = c(1, 2, 3), x1 = c(1, 2, 3))
  expect_error(survgbm(y ~ x1, data = dat), "Surv")
})
