...
test_that("all-constant predictors still fit without error", {
  ...
  time <- exp(rnorm(n))
  status <- rep(1L, n)
  fit <- survgbm(x, time = time, status = status, objective = "cox", ntrees = 5L, verbose = FALSE)
  p <- predict(fit, x)
...
test_that("a single-row dataset does not crash training or prediction", {
  ...
  fit <- survgbm(x, time = 3, status = 1L, objective = "cox", ntrees = 3L,
...
test_that("a tiny two-row dataset with min_node_size forcing no splits still yields finite leaf values", {
  ...
  time <- c(10, 20)
  status <- c(1L, 1L)
  fit <- survgbm(x, time = time, status = status, objective = "cox", ntrees = 5L,
...
test_that("AFT with near-perfectly separated risk groups stays numerically finite", {
  ...
  time <- exp(x[, 1] * 2)
  status <- rep(1L, n)
  fit <- survgbm(x, time = time, status = status, objective = "aft", ntrees = 50L,
