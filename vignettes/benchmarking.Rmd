...
`inst/benchmarks/run-benchmark.R` reproduces a benchmark of `survgbm` against `gbm`,
...
* `benchmark-summary.csv` -- median/IQR per (dataset, model), plus paired
  win/loss/tie counts of `survgbm` against each competitor;
...
`paper/survgbm-benchmark.qmd` (renders to `survgbm-benchmark.pdf` via `quarto render`).
